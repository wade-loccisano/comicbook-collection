<template>
  <div class="character-comic-details">
        <div class="search-details-results" v-if="hasSearched">
            ...
            <br />
            <div v-if="this.characters.length > 0">Characters: <span v-for="character in this.characters" v-bind:key="character.resourceURI">{{ character.name }}, </span></div>
            <br />
            <div >Description: {{ this.returnedComic.data.results[0].description }}</div>
            <br />
        ...
        <div class="return-to-collection-btn">
            <router-link :to="{name: 'character-comics', params: { id: $route.params.id, creatorId: $route.params.creatorId }}">
...
.search-details-results {
    ...
    align-items: center;
     font-size: 22px; line-height: 22px; padding: 8px; border-radius: 12px; font-family: 'Bangers', cursive;
     font-weight: normal; text-decoration: none; font-style: normal; font-variant: normal; text-transform: none;
}
.character-comic-details {
    display: flex;
    justify-content: center;
    flex-direction: column;
